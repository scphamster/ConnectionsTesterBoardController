...
void
USI_TWI_Master_Speed(uint8_t fm)
{
...
USI_TWI_Master_Initialise(void)
{
    ...
    USI_PUE |= (1 << USI_SDA_BIT);         // Pullup on SDA
    USI_CLOCK_PUE |= (1 << USI_SCL_BIT);   // Pullup on SCL - USI_CLOCK_* defines are to handle the case of SCL being on
                                           // a different port than SDA, which is true on....
#endif                                     // *drumroll* The 1634 - and nothing else!
    USI_PORT |= (1 << USI_SDA_BIT);        // Enable pullup on SDA, to set high as released state.
    USI_PORT |= (1 << USI_SCL_BIT);        // Enable pullup on SCL, to set high as released state.

    USI_DDR |= (1 << USI_SCL_BIT);   // Enable SCL as output.
    USI_DDR |= (1 << USI_SDA_BIT);   // Enable SDA as output.
...
USI_TWI_Start_Transceiver_With_Data_Stop(unsigned char *msg, unsigned char msgSize, unsigned char stop)
{
    ...
    USI_PORT |= (1 << USI_SCL_BIT);   // Release SCL.
    while (!(USI_PIN & (1 << USI_SCL_BIT)))
        ;   // Verify that SCL becomes high.
    if (USI_TWI_MASTER_SPEED)
        DELAY_T4TWI_FM;
    else
        DELAY_T2TWI;                   // Delay for T4TWI if TWI_FAST_MODE,  T2TWI if TWI_STANDARD_MODE
                                       /* Generate Start Condition */
    USI_PORT &= ~(1 << USI_SDA_BIT);   // Force SDA LOW.
    if (USI_TWI_MASTER_SPEED)
        DELAY_T4TWI_FM;
    else
        DELAY_T4TWI;
    USI_PORT &= ~(1 << USI_SCL_BIT);   // Pull SCL LOW.
    USI_PORT |= (1 << USI_SDA_BIT);    // Release SDA.
#ifdef SIGNAL_VERIFY
    if (!(USISR & (1 << USISIF))) {
    ...
    do {
        ...
        if (USI_TWI_state.addressMode || USI_TWI_state.masterWriteDataMode) {
            ...
            USI_DDR &= ~(1 << USI_SDA_BIT);            // Enable SDA as input.
...
---------------------------------------------------------------*/
unsigned char
USI_TWI_Master_Transfer(unsigned char temp)
{
...
---------------------------------------------------------------*/
unsigned char
USI_TWI_Master_Stop(void)
{
    USI_PORT &= ~(1 << USI_SDA_BIT);   // Pull SDA low.
    USI_PORT |= (1 << USI_SCL_BIT);    // Release SCL.
    while (!(USI_PIN & (1 << USI_SCL_BIT)))
        ;   // Wait for SCL to go high.
    if (USI_TWI_MASTER_SPEED)
