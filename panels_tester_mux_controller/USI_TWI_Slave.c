...
----------------------------------------------------------*/
void
USI_TWI_Slave_Initialise(unsigned char TWI_ownAddress)
{
    Flush_TWI_Buffers();
...
void
USI_TWI_Transmit_Byte(unsigned char data)
{
...
unsigned char
USI_TWI_Peek_Receive_Byte(void)
{
    unsigned char tmptail;
    unsigned char tmpRxTail;   // Temporary variable to store volatile
    tmpRxTail = TWI_RxTail;    // Not necessary, but prevents warnings
...
unsigned char
USI_TWI_Space_In_Transmission_Buffer(void)
{
    ...
    case USI_SLAVE_REQUEST_DATA:
        ...
        if (USI_TWI_On_Slave_Receive) {
            ...
            if (TWI_RxTail != tmpRxHead) {   // data in receive buffer
                ...
                while ((USISR & ((1 << USISIF) | (1 << USIPF) | (0xE << USICNT0))) == 0)
